...
| `evaluate.py`     | Standalone evaluation — **as-shipped it does NOT restore the trained checkpoint** (builds a fresh B=0 LoRA = base), so it measures the *base* model. Use `eval_ckpt.py` instead. |
...
1. **Model + optimizer** — Tunix uses Orbax. Pointing `CKPT_DIR` at a
   ...
   `/tmp` is volatile. Copy somewhere safe before restarting:
   ```bash
