...
def main():
    ...
    train_ds, val_ds, _ = build_train_val_test(
        ...
        second_source=SECOND_SOURCE, mix_alpha=MIX_ALPHA, mix_seed=MIX_SEED,
    )
    print(f"Datasets: train={len(train_ds)} val={len(val_ds) if val_ds else 0}  "
          f"CURRICULUM={CURRICULUM} FILTER=[{DIFFICULTY_MIN},{DIFFICULTY_MAX}] "
          f"MIX={SECOND_SOURCE or 'none'}@{MIX_ALPHA}")
