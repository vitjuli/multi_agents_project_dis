...
def _load_jsonl_split(data_dir: str, split: str) -> list:
    """Read pre-downloaded JSONL files from the TFDS downloads cache.

    TFDS caches both splits together; we distinguish train (7473) from test
    (1319) by line count.  Returns a list of {"question", "answer"} dicts.
    """
    import glob
    import json
    downloads_dir = os.path.join(data_dir, "downloads", "gsm8k")
    jsonl_files = sorted(glob.glob(os.path.join(downloads_dir, "*.jsonl")))
    if not jsonl_files:
        raise FileNotFoundError(
            f"No JSONL files found in {downloads_dir}. "
            "Run with source='tfds' once (or kaggle) to download data first."
        )
    want_large = (split == "train")  # train~7473, test~1319
    for path in jsonl_files:
        with open(path) as fh:
            lines = fh.readlines()
        if (len(lines) > 2000) == want_large:
            return [json.loads(l) for l in lines]
    raise FileNotFoundError(
        f"Could not find a JSONL file matching split='{split}' in {downloads_dir}"
    )
...
def get_dataset(data_dir: str, split: str = "train", source: str = "tfds") -> grain.MapDataset:
    ...
    if source == "jsonl":
        data = _load_jsonl_split(data_dir, split)
    elif source == "tfds":
