...
INNER="cd $REPO/R5_conditional_reward_length/scripts && source $VENV/bin/activate && python -u train.py"
if [[ "${1:-}" == "resume" ]]; then
  INNER="cd $REPO/R5_conditional_reward_length/scripts && source $VENV/bin/activate && WANDB_RUN_ID=$WANDB_RUN_ID python -u train.py --wandb-run-id $WANDB_RUN_ID"
fi
...
LOG="$REPO/R5_conditional_reward_length/scripts/train.log"
